...
timerArr[3] = setTimeout(() => {
    ...
    setTimeout(() => {
        ...
        for (let i = 0; i < walls.length; i++) {
            walls[i].style.opacity = 1; //恢复房间透明度
        }
    }, 250);
}, 4750);
...
getSetBgAutoRot.addEventListener("change", function () {
    if (getSetBgAutoRot.checked) {
        if (withBgBlo) {
            subtitle("主人是以房间和我为参考系呢？还是主人以自己为参考系呢？");
        } else {
            subtitle(
                "怎么感觉房间和我不是同步旋转的呢？主人没有让房间和人家同时开始旋转哦，把房间自动旋转关了重开吧！"
            );
        }
        onSwtBgAutoRot();
        onSwtBloAutoRot();
...
getSetBloAutoRot.addEventListener("change", function () {
    if (getSetBloAutoRot.checked) {
        if (withBgBlo) {
            withBgBlo = false;
            subtitle(
                "主人只想让人家一个人转呢！不过人家还是觉得和房间一起转比较好哦~"
            );
        } else {
            withBgBlo = true;
            subtitle(
        ...
        if (withBgBlo == false) {
            withBgBlo = true;
            subtitle("主人是想让人家和房间同时开始旋转才不让人家转的吗？");
        } else {
            withBgBlo = false;
            subtitle("主人把人家单独关闭了...");
        }
...
getSetBgFlRot.addEventListener("change", function () {
    if (getSetBgFlRot.checked) {
        onSwtBgFlRot();
        subtitle("房间现在不能被主人控制旋转了");
    } else {
        offSwtBgFlRot();
        subtitle("房间现在又能被主人控制了");
    }
});
...
getSetCubeFlRot.addEventListener("change", function () {
    if (getSetCubeFlRot.checked) {
        onSwtCubeFlRot();
        subtitle(
            "主人现在只能控制房间旋转了，人家是不会转的哦，除非主人开启了上一个选项，让房间也不能转了"
        );
    } else {
        offSwtCubeFlRot();
...
getSetPlayMode.addEventListener("change", function () {
    if (modeFlag) {
        ...
        subtitle("主人不会要去找蓝牙音频设备来测试吧？？？");
    } else {
        pattern.style.color = "rgb(0, 123, 255)";
        modeFlag = true;
        musicDelay = 250;
        pattern.innerHTML = '蓝牙<span style="color:#fff"> | <span style="color:rgb(220, 53, 69")>250ms</span>';
        display(
            '已为您切换为<span style="color: rgb(0, 123, 255);">蓝牙</span>模式'
        );
        subtitle("如果没连接蓝牙音频设备，主人会发现人家没有踩点音乐哦！");
    }
});

/********** 动画速度 **********/
var speed = 1;
var speed460_920 = 460;
var speed46_92 = 0.46;
var speed920_1840 = 920;
var speed92_180 = 0.92;
var speed368_736 = 3.68;
var speed920_2760 = 1840;
var SpeedX5 = 1.15;
var music = new Audio("MUSIC/standard.mp3");
...
radSpeedX5.addEventListener("mouseup", function () {
    ...
    radSpeedX5.style.transform = "translateX(0px) scale(.75)";
    setTimeout(() => {
        radSpeedX5.style.transform = "translateX(44px) scale(.75)";
        setTimeout(() => {
            radSpeedX5.style.transform = "translateX(44px) scale(1)";
        }, 500);
    }, 500);
    radSpeed1X.style.transform = "translateX(0px) scale(1.25)";
    setTimeout(() => {
        radSpeed1X.style.transform = "translateX(-44px) scale(1.25)";
        setTimeout(() => {
