...
var TVRoomAutoRot = true; //用于判断房间和小电视是否自动旋转，从而字幕消失后作出判断
var TVGod = false;
...
function MoveHereF5() {
    ...
    if (stopF5) {
        console.log('触发2');
        display("初次开启dance模式，请将鼠标移动到此处");
        alertCustom.addEventListener("mouseenter", function () {
            alertCustom.innerHTML =
                "由于初次访问可能会导致音画不同步，因此我将强制刷新当前网页来缓解浏览器压力，如果刚刚刷新过了，请直接点击这个弹窗取消刷新。当然，鼠标悬浮在弹窗上会暂停刷新，离开3秒后刷新，下次开启dance模式将不提示刷新，除非您手动刷新过浏览器";
        })
        setTimeout(() => {
            console.log('触发3');
            ...
            var timer2;
            alertCustom.addEventListener("mousedown", fn)
            ...
            function fn() {
                ...
                setTimeout(() => {
                    ...
                    display("已终止刷新");
                    clearTimeout(timer2);
                    alertCustom.removeEventListener("mousedown", fn)
                }, 250);
            };
            alertCustom.addEventListener("mouseenter", function () {
                clearTimeout(timer1);
                clearTimeout(timer2);
                alertCustom.removeEventListener("mouseenter", fn)
            });
            alertCustom.addEventListener("mouseleave", function () {
                if (stopF5) {
                    timer2 = setTimeout(() => {
                        location.reload();
                        alertCustom.removeEventListener("mouseleave", fn)
                    }, 4000);
                }
            });
        }, 500);
...
function TVFocusBehavior() { //小电视获取焦点时，控制行为
    TVOnceBlurSay = true;
    TVBlurMove = false;
    TVFocus = false;
    TVMove = false;
...
function TVBlurBehavior() { //小电视失去焦点时，控制行为
    TVWalkUpFlag = false;
    TVBlurMove = true;
    TVFocus = true;
    TVMove = false;
